...
void setup(void) {
  a_client = client_init("server", 1234, "username", "password");
...
{
  fail_unless(client_init(NULL, 1234, "username", "password") == NULL,
      "When given a NULL client arg, client_init should return NULL");
  transport_client *test_client = client_init("server", 1234, "username", "password");
  /*
...
{
  fail_unless(client_connect(NULL) == 0,
      "Passing a NULL client to client_connect should return a failure");
  fail_unless(client_connect(a_client) == 0,
...
START_TEST(test_transport_client_connected)
{
  ...
  fail_unless(client_connected(a_client) == 0,
...
START_TEST(test_transport_client_recv)
{
  ...
  //NULL client case
  fail_unless(client_recv(NULL, 1) == NULL,
  ...
  transport_message *msg = client_recv(a_client, 1);
  fail_if(msg != NULL,
  ...
  transport_client* other_client = client_init("server2", 4321, "username", "password");
